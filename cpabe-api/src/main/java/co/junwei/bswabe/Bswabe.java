...
public class Bswabe {
	...
	public static void setup2(BswabePub pub, bswabemsk2 msk) throws NoSuchAlgorithmException {
		Element alpha, beta_inv;
		...
		keyGen.init(256);
		msk.k1=keyGen.generateKey();
		msk.k2=keyGen.generateKey();
		msk.k3=keyGen.generateKey();
	}
	...
	public static bswabeprv2 keygen2(BswabePub pub, bswabemsk2 msk, String[] attrs)
			throws NoSuchAlgorithmException {
		bswabeprv2 prv = new bswabeprv2();
		Element g_r, r, beta_inv;
		Pairing pairing;
		prv.h=pub.f;
		/* initialize */
	...
	public static BswabePrv keygen1(BswabePub pub, BswabeMsk msk, String[] attrs,bswabeMsk msk1)
			throws NoSuchAlgorithmException {
		BswabePrv prv = new BswabePrv();
		//bswabeMsk msk1=new bswabeMsk();
		Element g_r, r, beta_inv,g_ac,t1,t2;
		Pairing pairing;
		//Element A;
		/* initialize */
		pairing = pub.p;
		...
		g_r = pub.gp.duplicate();
		g_r.powZn(r);
		g_ac=pub.gp.duplicate();
		g_ac.powZn(msk1.a.mul(msk1.c));
		g_ac.div(g_r);
		g_ac.powZn(beta_inv);
	...
	public static BswabeCphKey enc(BswabePub pub, String policy)
		...
		System.out.println("enc"+cph.cs.toString());
	...
	public static BswabeCphKey enc2(BswabePub pub, String policy,String word)
		...
		s.setToRandom();
		ha=pub.g.duplicate();
		h_rp = pub.p.getZr().newElement();
		elementFromString(h_rp, word);
//		System.out.println("hash"+h_rp.toString());
		ha.powZn(h_rp);
		ha.powZn(s);
		//System.out.println(s);
		cph.cs = pub.g_hat_alpha.duplicate();
		cph.cs.powZn(s); /* num_exps++; */
		m = pairing.pairing(pub.g, ha);
		cph.cs.mul(m);
		cph.c = pub.h.duplicate();
		cph.c.powZn(s); /* num_exps++; */
...
//		System.out.println(lol.toString());
	...
	public static BswabeCphKey enc1(BswabePub pub, String policy,Element e)
			throws Exception {
		BswabeCphKey keyCph = new BswabeCphKey();
		BswabeCph cph = new BswabeCph();
		Element s, m;
	...
	public static BswabeElementBoolean dec2(BswabePub pub, bswabeprv2 prv2,
										   BswabeCph cph,bswabetpd tpd) {
		Element t;
		Element m;
		BswabePrv prv = new BswabePrv();
		prv.d=prv2.d;
		prv.comps=prv2.comps;
		BswabeElementBoolean beb = new BswabeElementBoolean();
...
//		m.mul(t); /* num_muls++; */
	...
	public static bswabetpd gentpd(bswabeprv2 prv2, BswabePub pub2, String w) throws NoSuchAlgorithmException
	{
		bswabetpd tpd = new bswabetpd();
		Element h_rp,k_1,k;
		k_1=prv2.h.duplicate();
		k=prv2.d.duplicate();
		tpd.comps=prv2.comps;
		h_rp = pub2.p.getZr().newElement();
		elementFromString(h_rp, w);
//		System.out.println("hash"+h_rp.toString());
		k_1.powZn(h_rp);
		k.mul(k_1);
		tpd.t=k;
